...
const Avater = ({user,logOut}) => {
  ...
  const handleDropDown = () => {
   ...
   <div className="dropdown">
      ...
      ><img id="avatarButton" type="button" data-dropdown-toggle="userDropdown" data-dropdown-placement="bottom-start" class="w-10 h-10 rounded-full cursor-pointer" src={user.photoURL} alt="User dropdown"/>
      </button>
      ...
      <div><h4 className='text-2xl'>{user.displayName}</h4></div>
    ...
    <ul class="py-2 text-sm text-gray-700 dark:text-gray-200 " aria-labelledby="avatarButton">
      ...
      <li>
        <Link to={`/profile/${user.email}`}  class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Profile</Link>
      </li>
      <li>
        <Link to={`/settings/${user.email}`} href="#" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Settings</Link>
      </li>
